...
export const AuthModal = ({ onClose, initialMode = 'signup' }: { onClose: () => void; initialMode?: AuthMode }) => {
    ...
    const header = mode === 'login' ? 'Welcome Back' : "Save points, badges, and streak—so you can't lose them.";
    const subtext = mode === 'login' 
        ? 'Log in to continue tracking your stats and badges.'
        : "No reset. Everything you've earned stays.";
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content auth-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <button className="google-button google-button-primary" onClick={handleGoogleSignIn} disabled={loading}>
                    ...
                    <span>{mode === 'signup' ? 'Save with Google' : 'Continue with Google'}</span>
                </button>
                
                <div className="divider"><span>or</span></div>
                
                {!showEmailForm && mode === 'signup' ? (
                    <button className="email-toggle-link" onClick={() => setShowEmailForm(true)}>
                        Use email instead
                    </button>
                ) : (
                    <form onSubmit={handleEmailAuth}>
                        ...
                        <button className="button" type="submit" disabled={loading}>
                            <span>{loading ? '...' : (mode === 'login' ? 'Log in' : 'CREATE ACCOUNT')}</span>
                        </button>
